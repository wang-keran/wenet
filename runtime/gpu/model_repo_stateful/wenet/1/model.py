...
# 在这个模型中不行，这些参数是必须的，不能少传或者不传。
# 最多少传END参数，等待5秒后自动结束，但是这样会影响效率，所以最好都要传递进来
# 但是在其他模型中，可以根据需要来设置参数，也可以不设置这些参数，
# 比如非流式的GPU模型，当然非流式的也根本没办法使用这些参数因为不符合语法规则，但是流式模型确实可以选择其中的部分参数进行使用。
